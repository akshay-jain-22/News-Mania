...
import { Pencil, Trash2, Save, X, ExternalLink, Tag } from "lucide-react"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function NoteCard({ note, onDelete, onUpdate }: NoteCardProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      setIsSaving(true)
      console.log("Updating note:", note.id, { title, content, topic, isMarkdown })
      const updatedNote = await updateNote(note.id, { title, content, topic, isMarkdown })
        ...
        {isEditing ? (
          <div className="space-y-4">
            ...
            <div className="flex items-center space-x-2">
              <Switch id={`markdown-${note.id}`} checked={isMarkdown} onCheckedChange={setIsMarkdown} />
              <Label htmlFor={`markdown-${note.id}`}>Markdown Support</Label>
            </div>
          </div>
        ) : (
          <div className="space-y-2">
